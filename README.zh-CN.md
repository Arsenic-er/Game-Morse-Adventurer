...
当前 **v0.12.0** 加入首个可实际工作的台站配件 CW-500 音频滤波器。玩家可在商店以 300 信用点购买，在仓库的单配件槽装备或卸下；滤波器以 650 Hz 为中心、带宽 500 Hz，对待机背噪与收到的 CW 同时生效，并将接收噪声降低 35%。发射台会显示滤波状态，新通联日志会保存配件快照，v2 库存迁移也会安全兼容旧存档。共有 85 项自动测试，覆盖 CW 核心与自动键循环、接收音频滤波、练习系统、玩家主叫 QSO 状态机、CQ 回应概率、日志与结算、成就推导、商店经济、配件库存与装备、传播模型、地图投影及存档迁移规则。每次 `main` 更新都会自动测试并生成带 SHA-256 校验的 Windows x64 便携版，源码仓库不会提交 EXE 或构建目录。
