...
当前 **v0.9.1** 已加入信用点商店、持久化装备库存和受所有权保护的仓库换装，共有 56 项自动测试，覆盖 CW 核心、练习系统、QSO 状态机、日志与结算、商店经济、传播模型及存档迁移规则。每次 `main` 更新都会自动测试并生成带 SHA-256 校验的 Windows x64 便携版，源码仓库不会提交 EXE 或构建目录。
