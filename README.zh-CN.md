...
- 持久化 QSO 结果页与通联日志，记录呼号、地区、距离、RST、传播、设备、WPM、发报准确率、节奏与请求重发次数。
...
当前 **v0.15.0** 暂停新增设备，集中补全首次值守闭环。四语言简报和可持久化的三档任务引导会说明 CQ、守听、报告交换与日志保存；盲听阶段不会提前显示对方呼号或完整来报。发送 `AGN K` 会让同一台站按同一信道条件重发，不重抽传播、不增加格式错误次数，也不改变奖励。具体格式错误会保留玩家译码并允许持续修正；发送按钮不再强制重复播放刚才的发报，另有独立“回放输入”按钮。QSO 日志升级为 v2，新增重发次数，并把原先误称的“抄收正确率”改为实际含义“发报准确率”；旧存档会安全迁移。存档详情也会显示当前真正装备的电台。现有 96 项自动测试覆盖这些行为。每次 `main` 更新仍会生成带 SHA-256 校验的 Windows x64 便携版。
