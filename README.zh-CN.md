...
当前 **v0.9.2** 已将 Home 物体悬停效果改为区域滤镜高亮，移除错误的多边形描边，并加入可保存的自动键速度与 Z/X 长按连续发报。共有 62 项自动测试，覆盖 CW 核心与自动键循环、练习系统、QSO 状态机、日志与结算、商店经济、传播模型及存档迁移规则。每次 `main` 更新都会自动测试并生成带 SHA-256 校验的 Windows x64 便携版，源码仓库不会提交 EXE 或构建目录。
