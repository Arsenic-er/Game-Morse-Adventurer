...
当前 **v0.13.0** 将开始界面原有的占位提示升级为完整的四章节像素风台站手册，支持简体中文、繁体中文、日语和英语。手册会在游戏内说明存档建立、进入电台即开始监听与玩家先呼叫 CQ、手键和自动键操作、传播地图、天线及 CW-500 滤波器，不再依赖外部说明。通联台面原先容易误解的“删除”按钮也已改为明确的“清空输入”。v0.12 的 CW-500 配件、v2 库存迁移和 QSO 设备快照继续完整保留。共有 85 项自动测试，覆盖 CW 核心与自动键循环、接收音频滤波、练习系统、玩家主叫 QSO 状态机、CQ 回应概率、日志与结算、成就推导、商店经济、配件库存与装备、传播模型、地图投影及存档迁移规则。每次 `main` 更新都会自动测试并生成带 SHA-256 校验的 Windows x64 便携版，源码仓库不会提交 EXE 或构建目录。
