...
当前 **v0.11.0** 已将管理中心报纸入口实现为四语言成就档案，首批包含首次通联、5 次与 10 次通联、5,000 km DX、P2 或更弱传播通联及联系三个地区。NPC 回报在窗口焦点或音频中断后会自动重试，成功播放前不会错误推进通联；历史日志也会显示所选记录自己的地区与传播等级。共有 73 项自动测试，覆盖 CW 核心与自动键循环、练习系统、玩家主叫 QSO 状态机、CQ 回应概率、日志与结算、成就推导、商店经济、传播模型、地图投影及存档迁移规则。每次 `main` 更新都会自动测试并生成带 SHA-256 校验的 Windows x64 便携版，源码仓库不会提交 EXE 或构建目录。
