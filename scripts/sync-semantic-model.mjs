...
await writeFile(path.join(outputDirectory, outputModelName), modelBuffer);
await writeFile(path.join(outputDirectory, outputContractName), contractBuffer);

process.stdout.write(
  `Semantic model synced from ${release.repository}@${release.commit}: `
    + `${outputModelName} (${modelBuffer.length} bytes, ${actualDigest})\n`,
);
