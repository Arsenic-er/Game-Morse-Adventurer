...
目前 **v0.9.3** 已將錯誤的世界地圖替換為單一的 2:1 等距圓柱投影地圖，刪除重複的日本，並讓臺站標記與真實經緯度保持一致。共有 64 項自動測試，涵蓋 CW 核心與自動鍵循環、練習系統、QSO 狀態機、日誌與結算、商店經濟、傳播模型、地圖投影及存檔遷移規則。每次 `main` 更新都會自動測試並產生附 SHA-256 校驗的 Windows x64 可攜版，原始碼倉庫不提交 EXE 或建置目錄。
