...
目前 **v0.10.0** 已將通聯改為常開接收機流程：進入電臺即出現背景雜訊，玩家必須先發 CQ，傳播等級決定虛構電臺是否回應；收到回應前不會提前顯示對方呼號與傳播詳情。共有 68 項自動測試，涵蓋 CW 核心與自動鍵循環、練習系統、玩家主叫 QSO 狀態機、CQ 回應機率、日誌與結算、商店經濟、傳播模型、地圖投影及存檔遷移規則。每次 `main` 更新都會自動測試並產生附 SHA-256 校驗的 Windows x64 可攜版，原始碼倉庫不提交 EXE 或建置目錄。
