...
目前 **v0.9.2** 已將 Home 物件懸停效果改為區域濾鏡高亮，移除錯誤的多邊形描邊，並加入可儲存的自動鍵速度與 Z/X 長按連續發報。共有 63 項自動測試，涵蓋 CW 核心與自動鍵循環、練習系統、QSO 狀態機、日誌與結算、商店經濟、傳播模型及存檔遷移規則。每次 `main` 更新都會自動測試並產生附 SHA-256 校驗的 Windows x64 可攜版，原始碼倉庫不提交 EXE 或建置目錄。
