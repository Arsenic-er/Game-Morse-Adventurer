...
**v0.37.0**: El capítulo 5 ya está completo: incluye la secuencia narrativa NOVA—SORA—MORSE, mapa de luces e historial basados en el archivo, récords anuales, sellos, logros, orientación tras los fallos, identidades estables de persona / estación y pausas y salidas seguras. El capítulo 6 ya es jugable como corte vertical de «Estación portátil de la colina»: permite elegir uno de tres lugares ficticios, montar una radio, antena de hilo y batería prestadas, recuperarse de un intercambio débil, enviar el tráfico QTH / PWR / ANT obligatorio y liquidar una sola vez sin alterar el equipo permanente. El capítulo 7 solo incorpora la base: la página Personas y QSL, tarjetas inmutables vinculadas a personas y las decisiones fijas creer / pedir revisión / aplazar. El capítulo 7 completo y los capítulos 8–15 siguen planificados; las comunicaciones de emergencia y los sistemas narrativos posteriores no se presentan como implementados.
...
> La versión portátil para Windows es un **prototipo sin firma**; Windows puede mostrar un aviso de editor desconocido o SmartScreen. Comprueba el SHA-256 adjunto antes de ejecutarla. El **código fuente disponible** permite la revisión pública bajo una licencia **propietaria**; el código y el arte originales del juego no son software de código abierto.
...
- Tres ranuras de guardado locales con indicativo en mayúsculas de hasta siete caracteres, ubicación inicial fija, equipo intercambiable y dinero.
...
- Bucle de QSO ficticio iniciado por el jugador: el receptor se abre automáticamente, el jugador llama CQ, la propagación determina si responde una estación y un contacto correcto continúa con indicativos, RST, 73/SK, dinero y registro.
...
- Cada QSO liquidado guarda un desglose permanente en siete idiomas: 100 dinero base, +50 por guardia independiente, +75 por contacto de señal débil P0–P2, +20 por la primera región y +25 por un nuevo récord de distancia. La página de resultado y el libro de guardia de Home leen el mismo desglose schema v6; los registros antiguos conservan su total histórico sin pagos retroactivos.
...
- La liquidación de dinero atómica e idempotente impide recompensar más de una vez un QSO completado.
- La progresión de estación al estilo Kerbal separa los puntos tecnológicos de los dinero: 14 proyectos QSO cada vez más difíciles financian 28 nodos en radio, antenas, señal, energía y automatización. Los nodos implementados desbloquean compras; los futuros son visibles pero aún no consumen puntos.
...
La suite automatizada cubre el núcleo CW y la repetición del manipulador, el filtrado de audio del receptor, el motor de prácticas, los catálogos y conjuntos regionales de indicativos ficticios, la migración al esquema v3, el aislamiento y la recarga de preferencias, el bloqueo del selector, los resúmenes curriculares de cuatro modos, el aislamiento del conjunto de repaso de debilidades, la recuperación de debilidades, la persistencia del dominio y las invariantes de las lecciones formales; además de la máquina de estados QSO de recepción a ciegas, el orden estricto y las repeticiones `AGN K`, el historial de revisión operativa, la elegibilidad de la guardia independiente, la probabilidad de respuesta a CQ, los registros y resultados persistentes, la derivación de logros, la liquidación idempotente de dinero, la economía de la tienda, la propiedad y configuración de radios y accesorios, el modelo de propagación, la proyección del mapa y las reglas de guardado.

## Estado del proyecto

La versión **v0.33.0** sustituye la validación binaria del formato CQ por una simulación de copia remota. Cada llamada transmitida se compara con varias formas CQ válidas y después se combina con la identidad del indicativo, el orden, el ritmo, la velocidad del jugador, la propagación y la habilidad del operador que escucha. Diez indicativos ficticios usan siete perfiles experimentales con velocidades preferidas, habilidad de recepción, paciencia, rigor de procedimiento, demora, longitud de respuesta y estilo de consulta distintos. Una llamada clara recibe respuesta dirigida; una copia parcial puede producir `?`, `AGN?`, `QRZ?` o `QRS?`; una señal reconocible pero ilegible puede provocar un CQ general, mientras que la basura total o un operador lacónico pueden quedar en silencio. Los registros schema v6 conservan calidad CQ, puntuación de copia, resultado, perfil, WPM remoto y número de consultas. Se mantienen la protección de salida de v0.25 y los siete idiomas. El mismo modelo de copia remota evalúa ahora el reporte RST/73 del jugador: una copia parcial produce `AGN? K` o `QRS? K`, un reporte ilegible se puede repetir y solo una retransmisión copiada completa y liquida el QSO. En v0.28, cinco de las diez estaciones ficticias pueden hacer una pregunta opcional según su personalidad sobre potencia, QTH, tiempo, nombre ficticio o edad ficticia; las otras cinco conservan el intercambio breve. `AGN K` repite la pregunta, `SKIP K` o `73 K` la omite, y los registros schema v6 solo guardan la pregunta, el resultado y el número de repeticiones, nunca el texto de la respuesta. Las hojas de expresiones terminadas de MORSE, WANG, NOVA, DIEGO y SORA se archivan únicamente en el repositorio privado de recursos; no se distribuyen, cargan ni muestran durante un contacto por radio. En las fases de reporte e intercambio opcional, `QRS K`, `QRS PSE K` o `PSE QRS K` repite el mismo mensaje de la misma estación más despacio, conserva propagación y recompensas y se detiene en 5 WPM. v0.30 incorpora una base de operadores procedurales versionada y determinada por una semilla para una población conceptual de 100.000 operadores ficticios en JP, US, CN, DE, CH y FI. Reconstruye perfiles bajo demanda y está diseñado para que futuros guardados conserven solo una cabecera de mundo mínima y los cambios de NPC encontrados, garantiza indicativos `SIM` únicos de siete caracteres por semilla y ofrece `pnpm npc:generate` para previsualizar o exportar en flujo; todavía no participa en la selección de QSO reales. v0.31 añade un gesto de borrado durante la transmisión: siete puntos o rayas idénticos y consecutivos borran solo la entrada aún no enviada; seis elementos, una mezcla o una pausa de carácter no lo activan. v0.33.0 considera equivalentes la escritura compacta entre palabras, la división del indicativo causada por el ritmo y el final estándar `PSE K`; la personalidad y la propagación siguen decidiendo la copia de señales realmente corruptas.
...
- v0.33.0 sustituye los créditos por una cuenta de Dinero compatible con partidas antiguas, añade un registro atómico de recompensas únicas y amplía las medallas a Contacto, Expedición, Operación, Equipo y Personas. Los créditos antiguos se migran 1:1; las medallas ya obtenidas se conservan sin pagos retroactivos. El diseño implementable de `El registro inacabado` define 15 capítulos, calendario anual, ADN de misiones y el modo Estación Abierta posterior a la historia.
