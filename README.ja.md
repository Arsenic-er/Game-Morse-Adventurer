...
現在の **v0.9.1** はクレジット制ショップ、永続化装備インベントリ、所有権を検証する倉庫装備変更に対応し、CW コア、練習システム、QSO ステートマシン、ログと精算、ショップ経済、伝搬モデル、セーブ移行規則をカバーする 56 件の自動テストがあります。`main` の更新ごとに自動テストを実行し、SHA-256 付き Windows x64 ポータブル版を生成します。EXE と生成済みビルドはソースリポジトリに含めません。
