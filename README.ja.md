...
現在の **v0.9.2** は Home の不正確な輪郭線を廃止してオブジェクト範囲のフィルターハイライトに変更し、保存可能な 5–40 WPM のオートキー速度と Z/X 長押し連続送信に対応しました。CW コアとキーイングループ、練習システム、QSO ステートマシン、ログと精算、ショップ経済、伝搬モデル、セーブ移行規則をカバーする 62 件の自動テストがあります。`main` の更新ごとに自動テストを実行し、SHA-256 付き Windows x64 ポータブル版を生成します。EXE と生成済みビルドはソースリポジトリに含めません。
