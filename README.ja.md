...
現在の **v0.8.0** は永続化 QSO 結果とログブックに対応し、CW コア、練習システム、QSO ステートマシン、ログと精算、伝搬モデル、セーブデータ規則をカバーする 44 件の自動テストがあります。EXE と生成済みビルドはソースリポジトリに含めず、最終ビジュアル選定とリリース検証後に Windows ポータブル版を別途公開します。
