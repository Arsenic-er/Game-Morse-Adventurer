...
現在の **v0.9.3** は不正な世界地図を単一の 2:1 正距円筒図法マップに置き換え、重複していた日本を削除し、局位置マーカーを実際の緯度・経度に合わせました。CW コアとキーイングループ、練習システム、QSO ステートマシン、ログと精算、ショップ経済、伝搬モデル、地図投影、セーブ移行規則をカバーする 64 件の自動テストがあります。`main` の更新ごとに自動テストを実行し、SHA-256 付き Windows x64 ポータブル版を生成します。EXE と生成済みビルドはソースリポジトリに含めません。
