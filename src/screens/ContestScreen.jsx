...
import {
  CONTEST_DURATION_MILLISECONDS, CONTEST_MODES, CONTEST_PHASES, contestExchangeText,
...
import { CONTEST_TEXT, contestLeaveRisk } from "./contestText.js";
...
export function ContestScreen({ language, save, inputBlocked = false, onActivityRisk, onRunChange, onSettle, onBack }) {
  ...
  const targetText = targetFor(run);
  ...
  const canTransmit = [CONTEST_PHASES.RUN_PILEUP, CONTEST_PHASES.SP_POOL, CONTEST_PHASES.EXCHANGE].includes(run.phase);
    ...
    try {
      ...
      try {
        ...
        const response = await window.cwgameSystem?.interpretCwTraffic?.({
          ...
          peerCallsign: peer.callsign, pendingQuestion: run.phase === CONTEST_PHASES.EXCHANGE ? "CONTEST" : "CALLSIGN",
  ...
  function chooseMode(mode) { cw.clearInput(); update(selectContestMode(run, mode)); }
  ...
  async function settle() { const result = await onSettle(run); if (result?.settled || result?.reason === "ALREADY_SETTLED") setSettled(true); }
  function retry() { cw.clearInput(); setSettled(false); update(retryContestRun(run, nowIso())); }
  function listen() {
  ...
  return <main className="screen contest-screen" data-testid="contest-screen" data-simulation="fictional-five-minute-contest"
    ...
    <section className="contest-layout" aria-busy={inputBlocked || semanticBusy}>
      ...
      <article className="contest-console">
        ...
        {canTransmit && <section className="contest-keyer"><div><small>{save.keyType === "automatic" ? t.keyHintAutomatic : t.keyHintManual}</small><strong>{cw.analysis.decoded || "_"}</strong><span>{cw.analysis.wpm} WPM</span></div><div><button data-action="contest-clear" onClick={cw.clearInput}>{t.clear}</button><button data-action="contest-agn" onClick={() => transmit("AGN K")}>{t.agn}</button><button data-action="contest-qrs" onClick={() => transmit("QRS K")}>{t.qrs}</button><button data-action="contest-submit" disabled={!cw.analysis.pulseCount || semanticBusy || inputBlocked} onClick={() => transmit()}>{t.transmit}</button></div></section>}
        {lastError && <p className="contest-error"><Warning />{t[lastError] ?? lastError}</p>}
        {run.phase === CONTEST_PHASES.COMPLETED && <section className="contest-result"><CheckCircle size={46} /><h2>{t.completed}</h2><strong data-contest-grade={score.grade}>{score.grade.toUpperCase()}</strong><b>{score.score}</b><button data-action="contest-settle" disabled={inputBlocked || settled} onClick={settle}>{t.settle}</button></section>}
