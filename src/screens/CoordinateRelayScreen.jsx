...
import { COORDINATE_RELAY_TEXT, coordinateRelayLeaveRisk } from "./coordinateRelayText.js";
...
export function CoordinateRelayScreen({ language, save, inputBlocked = false, onActivityRisk, onRunChange, onSettle, onBack }) {
  ...
  function settle() { const result = onSettle(run); if (result?.settled || result?.reason === "ALREADY_SETTLED") setSettled(true); }
  ...
  return <main className="screen coordinate-relay-screen" data-testid="coordinate-relay-screen" data-simulation="fictional-pixel-grid"
    ...
    data-packet-check={run.packet.check} data-coordinate-paused={inputBlocked || !windowActive} data-portrait-visible="false">
    ...
    <section className="coordinate-relay-layout" aria-busy={inputBlocked || semanticBusy}>
      ...
      <article className="coordinate-relay-console">
        ...
        {inputPhase && <section className="coordinate-relay-input"><h2>{run.phase === COORDINATE_RELAY_PHASES.RELAY_PACKET ? t.relayPacket : t.readback}</h2><label>{t.hint}<input value={message} maxLength={256} disabled={inputBlocked || semanticBusy} onChange={(event) => setMessage(event.target.value.toUpperCase())} /></label><button data-action="coordinate-submit" disabled={inputBlocked || semanticBusy || !message.trim()} onClick={() => submit()}>{t.send}</button><div><button data-action="coordinate-agn" disabled={inputBlocked || semanticBusy} onClick={() => submit("AGN K")}>{t.agn}</button><button data-action="coordinate-qrs" disabled={inputBlocked || semanticBusy} onClick={() => submit("QRS K")}>{t.qrs}</button></div></section>}
        ...
        {run.phase === COORDINATE_RELAY_PHASES.COMPLETED && <section className="coordinate-relay-result"><CheckCircle size={44} /><h2>{t.completed}</h2><button data-action="coordinate-settle" disabled={inputBlocked || settled} onClick={settle}>{t.settle}</button></section>}
